...
static const dchar[2][] uniAlphaTable = [
  ['A', 'Z'],
  ['a', 'z'],
  [0x00AA, 0x00AA],
  [0x00B5, 0x00B5],
...
/// Returns non-zero if u is a Unicode alpha character.
...
out(found)
{
  ...
  bool inTable()
  {
    foreach (i, range; uniAlphaTable)
      if (range[0] <= u && u <= range[1])
        return true;
    return false;
  }
  assert(!!inTable() == !!found);
  }
}
body
{
...
unittest
{
  ...
  auto dchar[] unichars = [0x0C8C, 0x0E81, 0x0EBD, 0x0F88,
  ...
  foreach (u; unichars)
    assert(isUniAlpha(u), "expected char to be a unialpha");
}
